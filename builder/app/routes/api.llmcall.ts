...
async function llmCallAction({ context, request }: ActionFunctionArgs) {
  ...
  if (streamOutput) {
    ...
    try {
      ...
      logger.info(`DEBUG: Model "${modelDetails.name}" reasoning=${isReasoning} fixedTemperature=${fixedTemperature}`);
      ...
      const baseParams = {
        system: withSpanishAgentOutput(system),
        messages: [
          {
            role: 'user' as const,
            content: `${message}`,
      ...
      const finalParams = fixedTemperature ? { ...baseParams, temperature: 1 } : { ...baseParams, temperature: 0 };
