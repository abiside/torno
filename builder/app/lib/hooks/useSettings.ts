...
import {
  ...
  updateEventLogs,
  updatePromptId,
  assistantExperienceStore,
} from '~/lib/stores/settings';
import type { AssistantExperience } from '~/lib/stores/settings';
import { setAssistantExperienceWithUi } from '~/lib/stores/assistant-experience-ui';
...
import { DEFAULT_APP_LANGUAGE } from '~/lib/common/agent-locale';
...
export interface UseSettingsReturn {
  ...
  /** simple = human, short, non-technical answers + no-code workbench; technical = full assistant */
  assistantExperience: AssistantExperience;
  setAssistantExperience: (mode: AssistantExperience) => void;
...
export function useSettings(): UseSettingsReturn {
  ...
  const assistantExperience = useStore(assistantExperienceStore);
  ...
  const [settings, setSettings] = useState<Settings>(() => {
    ...
    return {
      ...
      language: storedSettings?.language || DEFAULT_APP_LANGUAGE,
  ...
  const setAssistantExperience = useCallback((mode: AssistantExperience) => {
    setAssistantExperienceWithUi(mode);
    logStore.logSystem(`Assistant experience: ${mode}`);
  }, []);
