...
import { isNoCodeMode } from '~/utils/product-mode';
...
export type AssistantExperience = 'simple' | 'technical';

export const getInitialAssistantExperience = (): AssistantExperience => {
  if (!isBrowser) {
    return isNoCodeMode() ? 'simple' : 'technical';
  }

  const stored = localStorage.getItem(SETTINGS_KEYS.ASSISTANT_EXPERIENCE);
...
export const assistantExperienceStore = atom<AssistantExperience>(getInitialAssistantExperience());
...
export const updateAssistantExperience = (mode: AssistantExperience) => {
  assistantExperienceStore.set(mode);
  localStorage.setItem(SETTINGS_KEYS.ASSISTANT_EXPERIENCE, mode);
};
