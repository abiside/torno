...
import { assistantExperienceStore, getInitialAssistantExperience } from '~/lib/stores/settings';
...
/** Shell/file/save actions executed via the global workbench queue (used for Simplicity preview overlay). */
export const workbenchQueuedMutationDepth = atom(0);

/** Streaming file chunks being applied (sampled path, Simplicity preview overlay). */
export const workbenchStreamingFileWriteDepth = atom(0);
...
export class WorkbenchStore {
  ...
  currentView: WritableAtom<WorkbenchViewType> =
    import.meta.hot?.data.currentView ?? atom(getInitialAssistantExperience() === 'simple' ? 'preview' : 'code');
  ...
  addToExecutionQueue(callback: () => Promise<void>) {
    this.#globalExecutionQueue = this.#globalExecutionQueue.then(async () => {
      workbenchQueuedMutationDepth.set(workbenchQueuedMutationDepth.get() + 1);

      try {
        await callback();
      } finally {
        workbenchQueuedMutationDepth.set(Math.max(0, workbenchQueuedMutationDepth.get() - 1));
      }
    });
  }
  ...
  async _runAction(data: ActionCallbackData, isStreaming: boolean = false) {
    ...
    if (data.action.type === 'file') {
      ...
      const targetView = assistantExperienceStore.get() === 'simple' ? 'preview' : 'code';

      if (this.currentView.value !== targetView) {
  ...
  actionStreamSampler = createSampler(async (data: ActionCallbackData, isStreaming: boolean = false) => {
    const trackStreamedFile = isStreaming && data.action.type === 'file';

    if (trackStreamedFile) {
      workbenchStreamingFileWriteDepth.set(workbenchStreamingFileWriteDepth.get() + 1);
    }
    ...
    try {
      ...
      if (trackStreamedFile) {
        workbenchStreamingFileWriteDepth.set(Math.max(0, workbenchStreamingFileWriteDepth.get() - 1));
      }
    }
