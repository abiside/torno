...
import { modelRequiresTemperatureOne } from './constants';
...
export async function createSummary(props: {
  ...
  const { messages, env: serverEnv, apiKeys, providerSettings, onFinish, assistantExperience = 'technical' } = props;
...
---
  ...
  const summarySystem =
    assistantExperience === 'simple' ? `${summarySystemCore}\n\n${getSimpleVoiceBrevityRules()}` : summarySystemCore;

  // select files from the list of code file from the project that might be useful for the current request from the user
  const resp = await generateText({
    system: withSpanishAgentOutput(summarySystem),
    prompt: `
