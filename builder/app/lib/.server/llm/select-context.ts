...
import { IGNORE_PATTERNS, modelRequiresTemperatureOne, type FileMap } from './constants';
import { DEFAULT_MODEL, DEFAULT_PROVIDER, PROVIDER_LIST } from '~/utils/constants';
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyBoltActions } from './utils';
import { createScopedLogger } from '~/utils/logger';
import { LLMManager } from '~/lib/modules/llm/manager';
import { withSpanishAgentOutput } from '~/lib/common/agent-locale';
import { getSimpleVoiceBrevityRules } from '~/lib/common/prompts/nocode-prompt';
...
export async function selectContext(props: {
  ...
  if (!lastUserMessage) {
    throw new Error('No se encontró ningún mensaje del usuario');
  }

  const fixedTemperature = modelRequiresTemperatureOne(modelDetails.name);

  const selectContextSystemCore = `
...
---
  ...
  const selectContextSystem =
    assistantExperience === 'simple'
      ? `${selectContextSystemCore}\n\n${getSimpleVoiceBrevityRules()}`
      : selectContextSystemCore;

  // select files from the list of code file from the project that might be useful for the current request from the user
  const resp = await generateText({
    system: withSpanishAgentOutput(selectContextSystem),
    prompt: `
        ${summaryText}
  ...
  if (!updateContextBuffer) {
    throw new Error('Respuesta no válida. Sigue el formato indicado para el contexto.');
  }
  ...
  if (totalFiles == 0) {
    throw new Error('No se pudieron seleccionar archivos para el contexto');
  }
