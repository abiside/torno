...
import { workbenchQueuedMutationDepth, workbenchStreamingFileWriteDepth, workbenchStore } from '~/lib/stores/workbench';
...
export const Preview = memo(({ setSelectedElement }: PreviewProps) => {
  ...
  const queuedMutationDepth = useStore(workbenchQueuedMutationDepth);
  const streamingFileWriteDepth = useStore(workbenchStreamingFileWriteDepth);
  const showSimpleRefreshOverlay =
    simpleExperience && (streaming || queuedMutationDepth > 0 || streamingFileWriteDepth > 0);
  ...
  return (
    <div ref={containerRef} className={`w-full h-full flex flex-col relative`}>
      ...
      <div className="relative flex-1 border-t border-bolt-elements-borderColor flex justify-center items-center overflow-auto">
        {showSimpleRefreshOverlay && (
          <div
            role="status"
            aria-live="polite"
            className="absolute inset-0 z-[35] flex flex-col items-center justify-center gap-3 bg-bolt-elements-background-depth-1/90 backdrop-blur-sm px-6 text-center"
          >
            <div className="i-svg-spinners:90-ring-with-bg text-bolt-elements-loader-progress text-3xl animate-spin" />
            <p className="text-sm font-medium text-bolt-elements-textPrimary">Actualizando el proyecto</p>
            <p className="text-xs text-bolt-elements-textSecondary max-w-sm">
              La vista previa se está refrescando; puede no reflejar los últimos cambios hasta que termine.
            </p>
          </div>
        )}
        <div
