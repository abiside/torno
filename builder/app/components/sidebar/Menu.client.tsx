...
export const Menu = () => {
  ...
  const deleteSelectedItems = useCallback(
    async (itemsToDeleteIds: string[]) => {
      ...
      if (errors.length === 0) {
        toast.success(
          deletedCount === 1 ? '1 chat eliminado correctamente' : `${deletedCount} chats eliminados correctamente`,
        );
      } else {
        toast.warning(`Se eliminaron ${deletedCount} de ${itemsToDeleteIds.length} chats. Fallaron ${errors.length}.`, {
          autoClose: 5000,
        });
      }
  ...
  const handleBulkDeleteClick = useCallback(() => {
    if (selectedItems.length === 0) {
      toast.info('Selecciona al menos un chat para eliminar');
      return;
    }
    ...
    if (selectedChats.length === 0) {
      toast.error('No se encontraron los chats seleccionados');
      return;
    }
          ...
          <div className="p-4 space-y-3">
            <div className="flex gap-2">
              ...
              <button
                ...
                aria-label={selectionMode ? 'Salir del modo selección' : 'Modo selección de chats'}
              >
            ...
            {selectionMode && (
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" onClick={selectAll}>
                  {selectedItems.length === filteredList.length ? 'Desmarcar todo' : 'Seleccionar todo'}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
          ...
          <div className="flex-1 overflow-auto px-3 pb-3">
            {filteredList.length === 0 && (
              <div className="px-4 text-gray-500 dark:text-gray-400 text-sm">
                {list.length === 0 ? 'No hay conversaciones previas' : 'Sin resultados'}
              </div>
            )}
