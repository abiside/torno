...
export const ImportFolderButton: React.FC<ImportFolderButtonProps> = ({ className, importChat }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const folderName = filteredFiles[0]?.webkitRelativePath.split('/')[0] || 'Carpeta sin nombre';
    setIsLoading(true);
    ...
    try {
      ...
      if (textFiles.length === 0) {
        ...
        toast.error('No hay archivos de texto en la carpeta seleccionada');
      ...
      if (binaryFilePaths.length > 0) {
        ...
        toast.info(`Omitiendo ${binaryFilePaths.length} archivos binarios`);
      }
      ...
      toast.success('Carpeta importada correctamente');
    } catch (error) {
