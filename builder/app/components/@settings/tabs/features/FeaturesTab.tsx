      ...
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            <div
              className={classNames(
                'p-2 rounded-lg text-xl shrink-0',
                'bg-bolt-elements-background-depth-3 group-hover:bg-bolt-elements-background-depth-4',
                'transition-colors duration-200',
                'text-purple-500',
              )}
            >
              <div className="i-ph:hand-heart" />
            </div>
            <div>
              <h4 className="text-sm font-medium text-bolt-elements-textPrimary">Assistant experience</h4>
              <p className="text-xs text-bolt-elements-textSecondary mt-0.5 max-w-xl">
                <strong>Simplicity</strong> keeps replies short and human, hides terminal controls, and prioritizes
                preview—ideal for non-technical users. <strong>Technical</strong> restores the full code-assistant
                layout and uses your Prompt Library choice for builds.
              </p>
            </div>
          </div>
          <div className="flex rounded-full p-1 bg-bolt-elements-background-depth-1 border border-bolt-elements-borderColor shrink-0">
            {(['simple', 'technical'] as const).map((mode) => (
              <button
                key={mode}
                type="button"
                onClick={() => {
                  setAssistantExperience(mode);
                  toast.success(mode === 'simple' ? 'Simplicity mode enabled' : 'Technical assistant mode enabled');
                }}
                className={classNames(
                  'px-3 py-1.5 text-xs font-medium rounded-full transition-colors',
                  assistantExperience === mode
                    ? 'bg-bolt-elements-item-backgroundAccent text-bolt-elements-item-contentAccent'
                    : 'text-bolt-elements-textSecondary hover:text-bolt-elements-textPrimary',
                )}
              >
                {mode === 'simple' ? 'Simplicity' : 'Technical'}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-bolt-elements-textSecondary mt-0.5">
              Choose a prompt from the library to use as the system prompt (build mode, Technical experience only).
            </p>
          </div>
          <select
            value={promptId}
